...
@testset "rectangles" begin
    ...
    regions = xor_geometry(alg, poly1, poly2)
    expected = [
        Polygon([(4.0, 4.0), (1.0, 4.0), (1.0, 3.0), (0.0, 3.0), (0.0, 0.0), (3.0, 0.0), (3.0, 1.0), (4.0, 1.0)],
...
@testset "concave <>" begin 
    ...
    regions =  difference_geometry(alg, poly1, poly2)
    expected = [
        Polygon([(0.0, 10.0), (1.0, 11.0), (0.0, 12.0), (-1.0, 11.0)]),
        Polygon([(6.0, 6.0), (2.0, 10.0), (1.0, 9.0), (4.0, 6.0), (1.0, 3.0), (2.0, 2.0)]),
        Polygon([(1.0, 1.0), (0.0, 2.0), (-1.0, 1.0), (0.0, 0.0)]),
    ]
    @test are_equivalent(regions, expected)
    regions =  difference_geometry(alg, poly2, poly1)
    expected = [
        Polygon([(2.0, 10.0), (3.0, 11.0), (2.0, 12.0), (1.0, 11.0)]),
        Polygon([(0.0, 2.0), (1.0, 3.0), (-2.0, 6.0), (1.0, 9.0), (0.0, 10.0), (-4.0, 6.0)]),
        Polygon([(1.0, 1.0), (2.0, 0.0), (3.0, 1.0), (2.0, 2.0)]),
    ]
    @test are_equivalent(regions, expected)
    # intersection
    regions =  intersect_geometry(alg, poly1, poly2)
    ...
    expected = [
        Polygon(
            [(6.0, 6.0), (2.0, 10.0), (3.0, 11.0), (2.0, 12.0), (1.0, 11.0), (0.0, 12.0), (-1.0, 11.0), (0.0, 10.0), (-4.0, 6.0), (0.0, 2.0), (-1.0, 1.0), (0.0, 0.0), (1.0, 1.0), (2.0, 0.0), (3.0, 1.0), (2.0, 2.0)],
            [
                [(4.0, 6.0), (1.0, 3.0), (-2.0, 6.0), (1.0, 9.0)],
                [(2.0, 2.0), (1.0, 1.0), (0.0, 2.0), (1.0, 3.0)],
                [(1.0, 9.0), (0.0, 10.0), (1.0, 11.0), (2.0, 10.0),]
            ]
        ),
    ]
    @test are_equivalent(regions, expected)
...
@testset "self-intersect & rectangle" begin
    ...
    regions =  intersect_geometry(alg, poly1, poly2)
    expected = [
        Polygon([(8.5, 0.0), (4.0, 0.0), (2.0, 2.0), (0.0, 0.0), (4.0, 0.0), (8.5, 0.0), (11.0, 0.0), (11.0, 2.0)])
    ]
    @test are_equivalent(regions, expected)
    ## Union
    regions =  union_geometry(alg, poly1, poly2)
...
@testset "Ring & square" begin
    ...
    regions =  difference_geometry(alg, poly2, poly1)
    expected = [
        Polygon([(2.0, 2.0), (3.0, 1.0), (8.0, 6.0), (3.0, 11.0), (1.0, 9.0), (5.0, 5.0)]),
    ...
    expected = [
        Polygon(
            ...
            holes=[
                [(2.0, 2.0), (1.5, 2.5), (4.0, 5.0), (0.5, 8.5), (1.0, 9.0), (5.0, 5.0)],
                [(0.0, 9.0), (0.5, 8.5), (-2.0, 6.0), (1.5, 2.5), (0.0, 1.0), (-4.0, 5.0)]
            ]
        ),
    ]
    @test are_equivalent(regions, expected)
...
@testset "Overlapping rings" begin
    ...
    regions =  difference_geometry(alg, poly2, poly1)
    expected = [
        Polygon([(3.0, 10.0), (7.0, 6.0), (3.0, 2.0), (2.5, 2.5), (2.0, 2.0), (3.0, 1.0), (8.0, 6.0), (3.0, 11.0), (1.0, 9.0), (1.5, 8.5)]),
        Polygon([(0.5, 8.5), (-2.0, 6.0), (1.5, 2.5), (2.0, 3.0), (-1.0, 6.0), (1.0, 8.0)]),
    ]
    @test are_equivalent(regions, expected)
    ## Intersection
    regions =  intersect_geometry(alg, poly1, poly2)
    expected = [
        Polygon([(1.5, 2.5), (2.0, 2.0), (2.5, 2.5), (2.0, 3.0)]),
        Polygon([(0.5, 8.5), (1.0, 8.0), (1.5, 8.5), (1.0, 9.0)]),
    ]
    @test are_equivalent(regions, expected)
    ## Union
    regions = union_geometry(alg, poly1, poly2)
    ...
    expected = [
        Polygon(
            ...
            holes=[
                [(3.0, 10.0), (7.0, 6.0), (3.0, 2.0), (2.5, 2.5), (5.0, 5.0), (1.5, 8.5)],
                [(2.0, 2.0), (1.5, 2.5), (2.0, 3.0), (2.5, 2.5)],
                [(0.5, 8.5), (-2.0, 6.0), (1.5, 2.5), (0.0, 1.0), (-4.0, 5.0), (0.0, 9.0)],
                [(2.0, 3.0), (-1.0, 6.0), (1.0, 8.0), (4.0, 5.0)],
                [(1.0, 8.0), (0.5, 8.5), (1.0, 9.0), (1.5, 8.5)]
            ]
        )
    ]
    @test are_equivalent(regions, expected)
...
@testset verbose=true "multi - appearing holes" begin
    ...
    expected = [
        Polygon(
            [(1.0, -0.8), (2.5, -0.8), (2.5, 1.0), (3.0, 1.0), (3.0, 2.0), (0.0, 2.0), (0.0, -1.0), (1.0, -1.0)];
            holes=[
                [(1.976923, 1.0), (2.5, 1.8), (2.5, 1.0)],
                [(1.0, -0.494118), (1.0, 0.0), (0.5, 0.0), (0.5, 0.5), (1.0, 0.5), (1.0, 1.0), (1.976923, 1.0), (1.65, 0.5), (2.0, 0.5), (2.0, 0.0), (1.323077, 0.0)],
                [(1.0, -0.8), (0.8, -0.8), (1.0, -0.494118)],
            ]
        )
    ]
    ...
    expected = [
        Polygon(
            ...
            holes=[
                ...
                [(1.976923, 1.0), (2.5, 1.8), (2.5, 1.0)],
                [(1.0, -0.494118), (1.0, 0.0), (1.323077, 0.0)],
                [(2.0, 0.5), (2.0, 0.0), (1.323077, 0.0), (1.65, 0.5)],
                [(1.65, 0.5), (1.0, 0.5), (1.0, 1.0), (1.976923, 1.0)],
                [(1.0, -0.8), (0.8, -0.8), (1.0, -0.494118)],
            ]
        ),
    ]
