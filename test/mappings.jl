...
using PolygonAlgorithms: cyclic_set_equality, are_equivalent_polygons
...
@testset "mappings" begin
    ...
    @testset "match holes" begin
        @testset "two squares with holes" begin
            polygons = [
                ...
                [(7.0, 3.0), (7.0, 0.0), (4.0, 0.0), (4.0, 3.0)],
            ]
            holes = [
        ...
        @testset "nested squares with holes" begin
            poly1 = [(0.0, 0.0), (0.0, 7.0), (7.0, 7.0), (7.0, 0.0)]
            poly2 = [(2.0, 2.0), (2.0, 5.0), (5.0, 5.0), (5.0, 2.0)];
            holes = [
    ...
    @testset "events to path" begin
        @testset "diamond" begin
            ...
            @test cyclic_set_equality(exteriors, expected)
            @test isempty(holes)
        end

        @testset "small gap" begin
            segments = [
                AnnotatedSegment((0.0, 0.0), (4.0, -4.0)),
                AnnotatedSegment((0.0, 0.0), (3.0, 3.0)),
                AnnotatedSegment((3.0, 3.0), (7.0, -1.0)),
        ...
        @testset "improper" begin
            ...
            exteriors, holes = segments_to_paths(segments; face_selection=SPLIT_FACES)
            @test cyclic_set_equality(exteriors, [[(3.0, 5.0), (2.0, 2.0), (5.0, 1.0)]])
            @test isempty(holes)
            exteriors, holes = segments_to_paths(segments; face_selection=MERGE_FACES)
            expected = [
        ...
        @testset "improper with hole" begin
            ...
            @test cyclic_set_equality(exteriors, expected_exteriors)
            @test cyclic_set_equality(holes, expected_holes)
        end
    end
